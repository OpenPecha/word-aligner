...
with open(source_out_file, 'w', encoding="utf-8") as source_out, open(target_out_file, 'w', encoding="utf-8") as target_out:
    for subdir in os.listdir(input_dir):
        ...
        if os.path.isdir(full_subdir_path):
            ...
            if len(source_files) != len(target_files):
                print(f"Warning: Mismatch in number of files in {full_subdir_path}. Skipping this folder.")
                continue
            
            for src_file, tgt_file in zip(sorted(source_files), sorted(target_files)):
                with open(os.path.join(full_subdir_path, src_file), 'r', encoding="utf-8") as src, open(os.path.join(full_subdir_path, tgt_file), 'r', encoding="utf-8") as tgt:
                    src_lines = src.readlines()
                    tgt_lines = tgt.readlines()

                    for src_line, tgt_line in zip(src_lines, tgt_lines):
                        src_line = word_tokenizer(clean_text(src_line))
                        tgt_line = syllable_tokenizer(clean_text(tgt_line, is_tibetan=True))
...
subprocess.run(["snt2cooc", cooc_file_source_target, source_path + ".vcb", target_path + ".vcb", source_path + "_" + target_path.split('/')[-1] + ".snt"])
subprocess.run(["snt2cooc", cooc_file_target_source, target_path + ".vcb", source_path + ".vcb", target_path + "_" + source_path.split('/')[-1] + ".snt"])

# Run mgiza with co-occurrence files
subprocess.run(["mgiza", "-S", source_path + ".vcb", "-T", target_path + ".vcb",
                "-C", source_path + "_" + target_path.split('/')[-1] + ".snt", "-o", os.path.join(data_dir, "alignment"),
                "-CoocurrenceFile", cooc_file_source_target])

# Function to read vcb files and return a dictionary
def read_vcb(vcb_file):
    vocabulary = {}
    with open(vcb_file, 'r', encoding='utf-8') as f:
        for line in f:
...
for alignment_file in alignment_files:
    ...
    with open(alignment_file, "r", encoding="utf-8") as af:
        ...
        for i in range(1, len(lines), 3):
            ...
            for align in alignments:
                ...
                grouped_source_words = ["".join(source_tokens[idx-1] for idx in group) for group in grouped_indices]
    ...
    print(f"Number of alignments captured after processing {alignment_file}: {len(word_alignments)}")
...
for target_word, source_phrases in word_alignments.items():
    ...
    word_alignments[target_word] = [f"{phrase}_{count}" for phrase, count in ordered_phrases]
